import type { DeadCodeIssue, DeadSeverity } from "../types.js";
import { DEAD_SCORE_START, DEAD_DEDUCTIONS } from "../types.js";

export function computeDeadScore(
  issues: readonly DeadCodeIssue[],
): number {
  let score = DEAD_SCORE_START;
  for (const issue of issues) {
    score -= DEAD_DEDUCTIONS[issue.severity] ?? 0;
  }
...
export function sortBySeverity(
  ...
  const order: Record<DeadSeverity, number> = {
...
export function totalRiskWeight(issues: readonly DeadCodeIssue[]): number {
  return issues.reduce((acc, i) => acc + (DEAD_DEDUCTIONS[i.severity] ?? 0), 0);
}
...
export function topIssueDensityFiles(
  ...
  const fileMap = new Map<string, { count: number; maxSeverity: DeadSeverity }>();
  const order: Record<DeadSeverity, number> = { CRITICAL: 0, HIGH: 1, MEDIUM: 2, LOW: 3 };
