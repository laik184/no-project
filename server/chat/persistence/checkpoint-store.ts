...
function saveSnapshot(checkpointId: string, files: Record<string, string>): void {
  ensureSnapshotsDir();
  const snap: StoredSnapshot = { checkpointId, files };
  fs.writeFileSync(snapshotPath(checkpointId), JSON.stringify(snap), 'utf8');
}

function loadSnapshot(checkpointId: string): StoredSnapshot | null {
  const p = snapshotPath(checkpointId);
  if (!fs.existsSync(p)) return null;
  try {
    return JSON.parse(fs.readFileSync(p, 'utf8')) as StoredSnapshot;
  } catch {
    return null;
  }
}
...
export const chatCheckpointStore = {
  ...
  async diffCheckpoints(id1: string, id2: string): Promise<SnapshotDiff> {
