...
export function recordRunComplete(runId: string, projectId: number, graphId: string, outcome: DagRunMetrics["outcome"]): void {
  ...
  console.log(`[dag-metrics] run=${runId.slice(0,8)} nodes=${m.completedNodes}/${m.totalNodes} retries=${m.totalRetries} waves=${m.parallelWaves} ms=${m.totalMs}`);
}
