 ...
 * Uses shared session-state store (no repository import — no layer skip).
 */
...
function publishToProject(projectId: number, frame: string): void {
  for (const conn of connectionPool.getByProject(projectId)) {
    ...
    if (!ok) {
      sessionStateStore.delete(conn.sessionId);
    }
  }
}
...
export function registerConnection(
  ...
  const hb = setInterval(() => {
    ...
    if (!ok) {
      sessionStateStore.update(sessionId, { closed: true });
      clearInterval(hb);
    } else {
      sessionStateStore.update(sessionId, { lastHeartbeat: Date.now() });
    }
