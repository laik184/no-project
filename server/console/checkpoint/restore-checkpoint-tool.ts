/**
 * server/console/checkpoint/restore-checkpoint-tool.ts
 *
 * Restores a checkpoint snapshot via CheckpointService.
...
export class RestoreCheckpointTool implements ConsoleTool<RestoreCheckpointInput, ConsoleToolResult<RestoreCheckpointOutput>> {
  ...
  async execute(input: RestoreCheckpointInput): Promise<ConsoleToolResult<RestoreCheckpointOutput>> {
    try {
      if (!input.sha?.trim()) {
