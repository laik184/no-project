/**
 * server/tools/checkpoint/create-checkpoint-tool.ts
 *
 * Creates a named checkpoint (git commit snapshot) via CheckpointService.
 *
...
export class CreateCheckpointTool implements Tool<CreateCheckpointInput, ToolResult<CreateCheckpointOutput>> {
  readonly id          = 'checkpoint.create';
  readonly description = 'Creates a named checkpoint snapshot of the current project state.';

  async execute(input: CreateCheckpointInput): Promise<ToolResult<CreateCheckpointOutput>> {
    try {
      if (!input.label?.trim()) {
