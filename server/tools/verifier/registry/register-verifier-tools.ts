...
export function registerVerifierTools(opts: { force?: boolean } = {}): void {
  ...
  const tools = [
    ...
    checkServerHealthTool, endpointValidatorTool, runtimeLogParserTool, crashDetectorTool, runtimeValidatorTool,
    failureRecoveryTool, rollbackValidatorTool, checkpointValidatorTool,
    schemaValidatorTool, outputValidatorTool, executionValidatorTool, dependencyValidatorTool, verificationValidatorTool,
  ...
  for (const tool of tools) {
    try {
      ...
      console.warn(`[register-verifier-tools] Skipped "${tool.name}": ${(err as Error).message}`);
    }
  }
