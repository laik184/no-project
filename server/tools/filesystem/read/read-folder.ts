...
export const readFolderTool: ToolDefinition = {
  ...
  handler: async (input, ctx: ToolExecutionContext) => {
    ...
    const result = scannerService.scanFolder(path, { maxDepth: 1, includeHidden, sandboxRoot: ctx.sandboxRoot });
    if (!result.ok) throw new Error(result.error ?? 'Failed to read folder');
