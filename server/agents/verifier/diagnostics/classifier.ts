import type { FailureCategory, ParsedError, ErrorSeverity } from '../types.ts';
import { deduplicateErrors } from '../utils.ts';
...
export function classifyError(raw: string, file?: string): ParsedError {
  return { message: raw.trim().slice(0, 500), severity: classifySeverity(raw), category: classifyFailure(raw), file, raw };
}
...
export function analyzeOutput(runId: string, output: string): ParsedError[] {
  return deduplicateErrors(parseLines(output).filter((l) => ERROR_LINE_PATTERN.test(l)).map((l) => classifyError(l)));
}

export function analyzeMultipleOutputs(runId: string, outputs: string[]): ParsedError[] {
  return deduplicateErrors(outputs.flatMap((o) => analyzeOutput(runId, o)));
}
