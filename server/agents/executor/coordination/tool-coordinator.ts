...
export function coordinateTerminal(
  task: ExecutionTask,
  sandboxRoot: string,
): RoutedStep {
...
export function coordinateFilesystem(
  task: ExecutionTask,
  sandboxRoot: string,
): RoutedStep {
  const input = task.input as Record<string, unknown>;
...
function adaptCodingInput(
  ...
  switch (toolName) {
    ...
    case "coding_generate_react_context":
      return {
        contextName: `${toPascalCase(derivedName)}Context`,
        goal,
        ...raw,
      };
    ...
    case "coding_generate_api_validation":
      return {
        route: `/${derivedName.toLowerCase()}`,
        resource: derivedName,
        goal,
        ...raw,
      };
...
export function coordinateTask(
  ...
  switch (task.kind as TaskKind) {
    case "terminal":
      return coordinateTerminal(task, sandboxRoot);
    case "filesystem":
      return coordinateFilesystem(task, sandboxRoot);
    case "coding":
      return coordinateCoding(task);
    case "verify":
      return coordinateVerify(task);
    case "browser":
      return coordinateBrowser(task);
    default: {
