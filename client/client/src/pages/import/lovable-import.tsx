  ...
  async function handleImport() {
    ...
    try {
      const res = await fetch("/api/import/git", {
        ...
        body: JSON.stringify({ repoUrl: repoUrl.trim(), visibility: privacy, source: "Lovable" }),
      });
  ...
  if (importId) {
    return (
      <ImportLoadingOverlay
        serviceName="Lovable"
        serviceColor="#ff6b6b"
        serviceIcon={<svg viewBox="0 0 24 24" fill="white" className="w-9 h-9"><path d="M12 21.593c-5.63-5.539-11-10.297-11-14.402 0-3.791 3.068-5.191 5.281-5.191 1.312 0 4.151.501 5.719 4.457 1.59-3.968 4.464-4.447 5.726-4.447 2.54 0 5.274 1.621 5.274 5.181 0 4.069-5.136 8.625-11 14.402z" /></svg>}
        steps={["Connecting to Lovable…", "Fetching project data…", "Migrating components…", "Transferring styles…", "Setting up workspace…"]}
        importId={importId}
        ...
        <div className="w-full max-w-lg">
            ...
            <div>
              <div className="flex items-center gap-3 mb-3">
                <div className="w-9 h-9 rounded-xl bg-[#ff6b6b] flex items-center justify-center">
                  <svg viewBox="0 0 24 24" fill="white" className="w-5 h-5"><path d="M12 21.593c-5.63-5.539-11-10.297-11-14.402 0-3.791 3.068-5.191 5.281-5.191 1.312 0 4.151.501 5.719 4.457 1.59-3.968 4.464-4.447 5.726-4.447 2.54 0 5.274 1.621 5.274 5.181 0 4.069-5.136 8.625-11 14.402z" /></svg>
                </div>
                <h1 className="text-2xl font-semibold text-white" data-testid="heading-lovable-import">Import from Lovable</h1>
              </div>
              <p className="text-sm text-muted-foreground">Export your Lovable project via GitHub and import it here.</p>
            </div>
          ...
          <div className="space-y-6">
            <div className="space-y-2">
              ...
              <p className="text-xs text-muted-foreground">Export from Lovable → GitHub, then paste the repo URL above.</p>
            </div>
            <div className="space-y-2">
            ...
            <div className="pt-2">
              <button data-testid="button-import-lovable" onClick={handleImport} disabled={loading || !repoUrl.trim()}
                ...
                <Download className="w-4 h-4" /> {loading ? "Starting…" : "Import from Lovable"}
              </button>
            </div>
          </div>
        </div>
