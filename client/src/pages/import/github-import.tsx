  ...
  async function handleImport() {
    ...
    try {
      const res = await fetch("/api/import/git", {
        ...
        body: JSON.stringify({ repoUrl: repoUrl.trim(), visibility: privacy, source: "GitHub" }),
      });
  ...
  if (importId) {
    return (
      <ImportLoadingOverlay
        ...
        steps={["Connecting to GitHub…", "Resolving remote refs…", "Cloning files…", "Writing to sandbox…", "Finalizing workspace…"]}
        importId={importId}
      ...
      <main className="flex-1 flex items-start justify-center px-4 sm:px-6 py-8 sm:py-14">
        <div className="w-full max-w-lg">
          <div className="flex items-start justify-between mb-7">
            <div>
              <div className="flex items-center gap-3 mb-3">
                ...
                <h1 className="text-2xl font-semibold text-white" data-testid="heading-github-import">Import from GitHub</h1>
              </div>
              <p className="text-sm text-muted-foreground">Paste a public repository URL to clone it into a new project.</p>
            </div>
            <a href="https://docs.github.com" target="_blank" rel="noopener noreferrer" className="flex items-center gap-1.5 text-xs text-muted-foreground hover:text-foreground transition-colors mt-1 flex-shrink-0">
          ...
          <div className="space-y-6">
                    ...
                    <button key={opt.value} onClick={() => setPrivacy(opt.value)} data-testid={`radio-${opt.value}`} className={cn("flex flex-col gap-2 p-4 rounded-xl text-left transition-all duration-150", active ? "border border-primary/50 bg-primary/10" : "border border-white/[0.08] bg-white/[0.03] hover:bg-white/[0.06]")}>
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          ...
                          <span className={cn("text-sm font-medium", active ? "text-white" : "text-muted-foreground")}>{opt.label}</span>
                        </div>
            ...
            <div className="pt-2">
              <button data-testid="button-import-github" onClick={handleImport} disabled={loading || !repoUrl.trim()}
                ...
                {loading ? "Starting…" : "Import from GitHub"}
              </button>
            </div>
          </div>
        </div>
