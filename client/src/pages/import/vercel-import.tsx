  ...
  async function handleImport() {
    ...
    try {
      const res = await fetch("/api/import/git", {
        ...
        body: JSON.stringify({ repoUrl: repoUrl.trim(), visibility: privacy, source: "Vercel" }),
      });
  ...
  if (importId) {
    return (
      <ImportLoadingOverlay
        serviceName="Vercel"
        serviceColor="#ffffff"
        serviceIcon={<svg viewBox="0 0 24 24" fill="white" className="w-9 h-9"><path d="M12 2L2 22h20L12 2z" /></svg>}
        steps={["Connecting to Vercel…", "Fetching project config…", "Migrating deployment settings…", "Pulling source files…", "Finalizing workspace…"]}
        importId={importId}
        ...
        <div className="w-full max-w-lg">
            ...
            <div>
              <div className="flex items-center gap-3 mb-3">
                <div className="w-9 h-9 rounded-xl bg-black border border-white/10 flex items-center justify-center">
                  <svg viewBox="0 0 24 24" fill="white" className="w-5 h-5"><path d="M12 2L2 22h20L12 2z" /></svg>
                </div>
                <h1 className="text-2xl font-semibold text-white" data-testid="heading-vercel-import">Import from Vercel</h1>
              </div>
              <p className="text-sm text-muted-foreground">Paste your project's GitHub URL to clone the source code.</p>
            </div>
          ...
          <div className="space-y-6">
            <div className="space-y-2">
              ...
              <p className="text-xs text-muted-foreground">Or select a project from your connected account below.</p>
            </div>
            <div className="space-y-2">
            ...
            <div className="pt-2">
              <button data-testid="button-import-vercel" onClick={handleImport} disabled={loading || !repoUrl.trim()}
                ...
                <Download className="w-4 h-4" /> {loading ? "Starting…" : "Import from Vercel"}
              </button>
            </div>
          </div>
        </div>
