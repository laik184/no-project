...
export function collectSearchExpanded(nodes: RawTreeNode[], basePath: string, sq: string, result: Set<string>): boolean {
  let anyMatch = false;
...
export function InlineCreateRow({ type, onConfirm, onCancel }: { type: "file" | "folder"; onConfirm: (name: string) => void; onCancel: () => void }) {
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 6, padding: "2px 6px 2px 8px", height: 22, borderBottom: "1px solid #222", background: "#1e1e1e" }}>
      {type === "file" ? <FilePlus style={{ width: 11, height: 11, color: "#60a5fa", flexShrink: 0 }} /> : <FolderPlus style={{ width: 11, height: 11, color: "#e8a427", flexShrink: 0 }} />}
...
export function RenderNode({
  ...
  const rowStyle: React.CSSProperties = {
    display: "flex", alignItems: "center", gap: 4, height: 20, paddingRight: 4, paddingLeft,
